...
${articleContext}`;
        aiSummary = (await callConfiguredAI(env, { prompt: `${prompt}\n\nUse only this evidence. Include chronology, exact named actors and figures, implications, counter-evidence, limitations, and source titles. Do not invent missing facts.`, max_tokens: 3500, temperature: 0.2, response_profile: 'deep-analysis' })) || '';
    } catch (e) {
...
${context}`;
        aiAnalysis = (await callConfiguredAI(env, { prompt: `${prompt}\n\nProduce a detailed evidence record: market context, dated developments, actors, regulatory implications, operational constraints, counter-evidence, limitations, and further diligence. Do not estimate absent figures.`, max_tokens: 3500, temperature: 0.2, response_profile: 'deep-analysis' })) || '';
    } catch (e) {
