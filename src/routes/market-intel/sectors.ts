...
router.get('/sector/:id', async (c) => {
    ...
    if (recentArticles.results && recentArticles.results.length > 0) {
        ...
        try {
            ...
            const response = await callConfiguredAI(c.env, { prompt: `${prompt}\n\nSynthesize only supported evidence in depth: dated developments, companies and institutions named, regulatory context, operational implications, counter-evidence, limitations, and diligence questions.`, max_tokens: 3200, temperature: 0.2, response_profile: 'deep-analysis' });
        ...
        ai_trend_analysis: await getCached(
            ...
            async () => {
                try {
                    ...
                    const aiResponse = await callConfiguredAI(c.env, { prompt: `${prompt}\n\nProvide a detailed evidence synthesis with chronology, actors, implications, contradictions, limitations and source gaps.`, max_tokens: 3200, temperature: 0.2, response_profile: 'deep-analysis' });
